...
O PrintConnect também possui um sistema próprio de atualização automática.

---

## 🚀 Instalação

1. Clique em **BAIXAR PRINTCONNECT PARA WINDOWS** acima.
2. Na página da Release, baixe o arquivo **PrintConnect-Setup-0.5.1.exe**.
...
O PrintConnect verifica novas versões diretamente pelo GitHub e permite realizar a atualização pelo próprio aplicativo.
...
Quando uma nova versão é publicada, o aplicativo verifica automaticamente se existe uma versão mais recente.

### Processo de atualização

1. O PrintConnect verifica a versão disponível.
2. Uma nova versão é identificada.
3. O usuário seleciona **Atualizar agora**.
4. O Dashboard é encerrado.
5. O serviço é interrompido com segurança.
6. É realizado um backup da instalação.
7. O Agent é atualizado.
8. O Desktop é atualizado.
9. O serviço é iniciado novamente.
10. A nova versão é validada.
11. O Dashboard é iniciado novamente.

O processo possui mecanismos de segurança para reduzir o risco de uma atualização incompleta deixar o sistema indisponível.
...
Isso permite preservar a fila mesmo após:

- Reinicialização do computador
- Reinicialização do serviço
- Interrupção temporária da impressora

Quando o serviço volta a funcionar, os trabalhos pendentes podem continuar sendo processados.
...
Também possui uma ferramenta de **Diagnóstico Bluetooth** para auxiliar na identificação de problemas de comunicação.
...
**PrintConnect**  
↓  
**Impressora Windows ou Bluetooth**
...
⬇️ **[VER VERSÕES DISPONÍVEIS](https://github.com/MarcosViniciusF/PrintConnect-Download/releases)**
